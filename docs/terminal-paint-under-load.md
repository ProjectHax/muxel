...
| `MUXEL_PROFILE_TERMINAL=1` | terminal key→PTY-output→paint only (`profile.rs`) |
...
| `MUXEL_UI_PRIORITY=above-normal` | raise the UI thread for an A/B test; requires `MUXEL_PROFILE=1` |
...
shape reuse, `key→pty-output` vs `pty-output→paint`, cursor-row probe.
...
The `ui-prof[v3 startup]` record identifies the executable, build commit,
dirty state, actual GPUI thread ID, effective priority, and priority request.
...
with shape-reuse %, felt-latency samples (`key→pty-output` = agent+ConPTY side,
`pty-output→paint` = muxel side), sync-expiry count, and a focused-pane cursor-row
