...
//! samples: `key→pty-output` (keypress until the focused pane's PTY output is parsed —
//! high here = ConPTY/agent/scheduling, not paint) and `pty-output→paint` (parsed
...
fn dump(tag: &str) {
    ...
    let interesting = keys > 0
        || spikes_3 > 0
        || spikes_8 > 0
        || echo_max >= 80_000 // ≥80ms key→pty-output
        || plat_max >= 30_000 // ≥30ms pty-output→paint
    ...
    // v6: v5 with precise names: PTY output need not be an input echo.
    let line = format!(
...
pub fn key_handled(held: bool, elapsed: Duration) {
    ...
    // Arm the key→pty-output latency sample with the FIRST unanswered key (a later
...
pub fn process_output(bytes: usize, elapsed: Duration, focused: bool) {
    ...
    // Focused-pane output: close key→pty-output and arm pty-output→paint. Only the
