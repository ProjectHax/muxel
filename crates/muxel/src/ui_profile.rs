...
static UI_ACTIVATED: AtomicBool = AtomicBool::new(false);
...
/// Name and record the actual GPUI dispatch thread. Priority elevation is an
/// explicit profiling experiment, never normal application behavior.
#[cfg(windows)]
pub fn activate_ui_thread() {
    use windows::Win32::System::Threading::{
        GetCurrentThread, GetCurrentThreadId, GetThreadPriority, SetThreadDescription,
        SetThreadPriority, THREAD_PRIORITY_ABOVE_NORMAL,
    };
    use windows::core::PCWSTR;

    if UI_ACTIVATED.swap(true, Ordering::SeqCst) {
        return;
    }
    ...
    let priority_result = match priority_request.as_deref() {
        Some("above-normal") if umbrella_profile_enabled() => {
    ...
    let started_unix_ms = std::time::SystemTime::now()
        ...
        .as_millis();
    emit(&format!(
        "ui-prof[v3 startup] pid={} ui_tid={} profiler_start_unix_ms={} \
         exe={executable:?} build_profile={} commit={} dirty={} priority={} \
         priority_request={priority_request:?} priority_result={priority_result}",
        std::process::id(),
        ...
        env!("MUXEL_BUILD_COMMIT"),
        env!("MUXEL_BUILD_DIRTY"),
        priority,
    ));
}
